...
Item {
	...
	QtObject {
		...
		property NlalertThreeStateButton prvButUp: butUp
		property NlalertThreeStateButton prvButDown: butDown
		...
		function updateButtons() {
			...
			//had to do manual setting of visible property of buttons here because there is a problem with icon in NlalertThreeStateButton when directly bound
		...
		function updateScrollbar() {
			...
			scrollbar.height = Math.floor(nlalertMarkup.height / pageCount);
			...
			topMargin = (topMargin + scrollbar.height) >= nlalertMarkup.height ? nlalertMarkup.height - scrollbar.height - 1 : topMargin;
	...
	Rectangle {
		...
		Column {
			id: repeaterColumn
			width: nlalertMarkup.width
			height: nlalertMarkup.height
			anchors {
				top: nlalertMarkup.top
				topMargin: 6
				left: nlalertMarkup.left
				leftMargin: 6
			}
			spacing: 5
	...
	NlalertThreeStateButton {
		id: butDown
		width: 38
		height: nlalertMarkup.height / 2
		...
		anchors {
			bottom: nlalertMarkup.bottom
			left: nlalertMarkup.right
